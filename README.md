...
- The "Article of the Week" spotlight refreshes every **Saturday** (cached in `data/spotlight.json`, keyed off the most recent Saturday's date). Candidates are ranked by study type/evidence level first, citation count second (`impact_score()` — the same deterministic scoring behind each article's star rating), not by citations alone: RCTs, meta-analyses/systematic reviews, and practice guidelines are pulled in from every organ-system category regardless of citation count (a brand-new landmark RCT hasn't had time to accumulate citations yet), on top of the citation-ranked "Trending this month" pool. If Groq is configured, an AI prompt can refine the pick from that shortlist; if Groq is absent, rate-limited, or fails, the top deterministic candidate is used instead so the Saturday rotation never stops.
- If `GROQ_API_KEY` is set, each article/post gets an AI-generated plain-language summary, a "why it matters" clinical significance note, and a one-line "🩺 Rounds takeaway" (a concrete, actionable bedside takeaway, left blank if the text doesn't support one) via Groq's free API. **Randomized Controlled Trial** articles get a different, much richer treatment instead: a full structured EBM critical appraisal (`ai_appraise_trial()`), in the style of thebottomline.org.uk — Clinical question, Background, Design, Setting, Population, Intervention, Management common to both groups, Intervention fidelity, Outcome, Author's conclusion, Strengths, Weaknesses, and The Bottom Line — since that format presupposes an actual RCT and isn't a good fit for reviews, guidelines, or observational studies. Results are cached in `data/ai_cache.json` so each article is only processed once, ever — not reprocessed every day it stays in the window.
- A dedicated "✅ Trial Results" section pulls every published trial-shaped article — Randomized Controlled Trial and Clinical Trial — out of every category/section into one consolidated list, separate from Trial Tracker (ClinicalTrials.gov *registrations*, which aren't results yet) and from the curated ICU Classics landmark trials (their own dedicated shelf, kept out so they don't mix with the current literature feed). A "Trials" quick-filter chip does the same aggregate match in place across every other section too, alongside the existing single-study-type chips (RCT, Meta-Analysis, Review, Guideline). Both deliberately exclude the "Multicenter Study" pubtype — it only means multiple study sites, not randomization, so it also tags plain multicenter cohort studies.
- A dedicated "🇸🇦 KSA Research" section surfaces ICU/critical-care articles actually led from Saudi Arabia, not the journal allowlist (same reasoning as Guideline Watch — requiring it would leave this section nearly empty). PubMed's `[AD]` (Affiliation) field tag is used only as a candidate pre-filter, since it matches *any* author's affiliation — a large multi-national trial or review with a long author list would otherwise get flagged just because one minor co-author is Saudi-affiliated. Each candidate is then confirmed by checking that the *first* author's affiliation (the standard proxy for where a study was primarily led) actually mentions Saudi Arabia. These articles are pulled *out* of whichever organ-system category they'd otherwise also land in, so each article lives in exactly one place instead of being duplicated.
- Each organ-system category can be hidden entirely (✕ button on its heading) if you're not interested in it — distinct from the collapse/expand toggle, which still shows the heading. Hidden categories persist across visits (localStorage) and reappear via a small restore chip in a "🚫 Hidden" bar, which only shows up once at least one category is hidden.
- A "🗓️ This Week's Articles" shelf pulls anything published in the last 7 days out of its organ-system category into one place — recomputed fresh from each article's own pubdate on every run, so an article "graduates" back into its category automatically once it's no longer this recent, with nothing to track or expire manually. It's purely a reorganization of already-fetched data, not an extra PubMed query.
...
ICU Scope is a static site, so Vercel can host it directly without a build step. For automatic literature updates, deploy it from the same GitHub repository rather than uploading a one-off ZIP: the GitHub Action updates the JSON files and commits them, and the new commit then triggers a Vercel redeploy. A direct ZIP deployment on Vercel will render correctly but will remain frozen at the data included in that ZIP.

A `vercel.json` file is included to keep the JSON feed and frontend assets revalidated instead of being held behind long browser/CDN caches.
