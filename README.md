...
- A dedicated "🇸🇦 KSA Research" section surfaces ICU/critical-care articles with a Saudi Arabia author affiliation, matched via PubMed's `[AD]` (Affiliation) field tag rather than the journal allowlist (same reasoning as Guideline Watch — requiring it would leave this section nearly empty). These articles are pulled *out* of whichever organ-system category they'd otherwise also land in, so each article lives in exactly one place instead of being duplicated.
